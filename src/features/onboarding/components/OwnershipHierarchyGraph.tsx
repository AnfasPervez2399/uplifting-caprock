...
const labelFor = (type: PartyType) =>
  type === "individual" ? "Individual" : type === "trust" ? "Trust" : "Corporate";
...
function LayerBadge({ subjects, label }: { subjects: Array<{ percentage: string }>; label: string }) {
  const complete = hasCompletePercentageLayer(subjects);
  return (
    <span className={`ownership-layer-badge inline-flex items-center gap-1.5 rounded-full border bg-white px-2.5 py-1 text-[8px] font-black uppercase tracking-[0.07em] shadow-sm ${complete ? "border-emerald-200 text-emerald-700" : "border-amber-200 text-amber-700"}`}>
...
function OwnershipNode({
  ...
  const required = subject.type === "individual" || requiresShareholderApplication(subject);
    ...
    <div className="flex min-w-[196px] flex-col items-center">
      ...
      >
        ...
        <span className="flex items-start gap-2.5 pl-1 pr-3">
          <span className={`relative grid h-9 w-9 shrink-0 place-items-center rounded-xl transition duration-300 group-hover:scale-105 ${tone.icon}`}>
            <Icon className="h-4 w-4" />
            {children.length ? <span className="absolute -bottom-1 -right-1 grid h-[18px] min-w-[18px] place-items-center rounded-full border-2 border-white bg-[#0f172a] px-1 text-[7px] font-black text-white">{children.length}</span> : null}
          </span>
  ...
  return (
    <>
      ...
      <div className="relative flex items-start justify-center gap-4 px-2">
        {subjects.length > 1 ? <div className="ownership-line-x absolute left-[106px] right-[106px] top-0 h-px origin-center bg-slate-300" /> : null}
        ...
        {subjects.length ? (
          <>
            ...
            <div className="relative flex items-start justify-center gap-5 px-2">
              {subjects.length > 1 ? <div className="ownership-line-x absolute left-[108px] right-[108px] top-0 h-px origin-center bg-slate-300" /> : null}
    ...
    <section className="overflow-hidden rounded-[22px] border border-slate-200 bg-[#f7f9fb] shadow-[0_9px_28px_rgba(15,23,42,0.045)]">
      <div className="border-b border-slate-200 bg-white px-4 py-4 sm:px-5">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-1.5 border-t border-slate-100 pt-3 text-[8px] font-black uppercase tracking-[0.06em]">
          ...
          <span className="rounded-full bg-[#dce7f2] px-2.5 py-1 text-[#003478]">Corporate</span>
          <span className="rounded-full bg-[#f5e8c9] px-2.5 py-1 text-[#65480f]">Trust</span>
