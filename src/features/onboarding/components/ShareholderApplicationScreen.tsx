...
function ApplicationFlow<T extends ApplicationSubject>({
  ...
  const [activeOwnerId, setActiveOwnerId] = useState<string | null>(initialOwnerPath[0] || null);
  ...
  if (activeOwner) {
    return (
      <ApplicationFlow
        subject={activeOwner}
        initialOwnerPath={initialOwnerPath.slice(1)}
        onChange={(nextOwner) => {
          onChange({
            ...subject,
            application: {
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-[1440px] items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
          ...
          <div className="min-w-0 text-center">
            <p className="truncate text-sm font-semibold text-slate-950">{getTypeLabel(subject)} shareholder application</p>
            <p className="mt-0.5 text-[10px] font-bold uppercase tracking-[0.1em] text-[#003478]">Ownership layer {depth}</p>
          </div>
          <button type="button" onClick={onExit} aria-label="Close shareholder application" className="grid h-10 w-10 place-items-center rounded-xl text-slate-500 transition hover:bg-slate-100 hover:text-slate-900"><X className="h-5 w-5" /></button>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-[1440px] px-4 py-5 sm:px-6 sm:py-8 lg:px-8">
        ...
        <div className="grid gap-6 lg:grid-cols-[270px_minmax(0,1fr)]">
          <aside className="h-fit rounded-[22px] border border-slate-200 bg-white p-3 shadow-sm lg:sticky lg:top-24">
            <p className="px-3 pb-2 pt-1 text-[10px] font-bold uppercase tracking-[0.13em] text-slate-400">{getTypeLabel(subject)} application</p>
            <nav className="space-y-1" aria-label="Shareholder application sections">
              {steps.map((item, index) => {
                ...
                return (
                  <button key={item.id} type="button" onClick={() => { setScreen(item.id); setScreenError(""); }} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold transition ${active ? "bg-[#dce7f2] text-[#0f172a]" : "text-slate-600 hover:bg-slate-50 hover:text-slate-950"}`}>
                    <span className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg ${active ? "bg-white/75 text-[#003478]" : "bg-slate-100 text-slate-500"}`}>{complete ? <Check className="h-4 w-4" strokeWidth={2.5} /> : <Icon className="h-4 w-4" />}</span>
                    <span className="min-w-0 flex-1"><span className="block truncate">{item.label}</span><span className="mt-0.5 block text-[10px] font-medium text-slate-400">Section {index + 1} of {steps.length}</span></span>
                  </button>
                );
              })}
            </nav>
            <div className="mt-3 rounded-xl bg-slate-50 px-3 py-3 text-[10px] leading-5 text-slate-500">Cash Accounts and Link External Account stay with the main application and are not repeated here.</div>
          </aside>
          ...
          <main className="min-w-0 rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
            ...
            {screen === "ownership" ? <OwnershipScreen subject={subject} updateApplication={updateApplication} openOwner={setActiveOwnerId} /> : null}
            {screen === "identity" ? <IdentityScreen subject={subject} updateApplication={updateApplication} onOpenDocument={onOpenDocument} /> : null}
            ...
            {screen === "review" ? <ReviewScreen subject={subject} steps={steps} updateApplication={updateApplication} /> : null}
            ...
            <div className="mt-8 flex flex-col-reverse gap-3 border-t border-slate-100 pt-6 sm:flex-row sm:items-center sm:justify-between">
              ...
              {screen === "review" ? (
                ...
                <button type="button" onClick={goForward} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#003478] px-5 text-sm font-semibold text-white transition hover:bg-[#002b63]">Continue <ArrowRight className="h-4 w-4" /></button>
              )}
            </div>
          </main>
        </div>
  ...
  return (
    <div className="animate-[fadeUp_.25s_ease-out]">
      ...
      <div className="mt-7 grid gap-5 sm:grid-cols-2">
        ...
        <Field label="Email" htmlFor={`owner-email-${subject.id}`}><input id={`owner-email-${subject.id}`} type="email" value={subject.email} onChange={(event) => updateSubject({ email: event.target.value })} placeholder="name@example.com" className={inputClass()} /></Field>
...
function TrustPartiesScreen({ kind, subject, updateApplication }: { kind: "trustees" | "beneficiaries"; subject: ApplicationSubject; updateApplication: (patch: Partial<ShareholderApplication>) => void }) {
  ...
  return (
    <div className="animate-[fadeUp_.25s_ease-out]">
      <ScreenHeading eyebrow="Related parties" title={title} description={`Add every ${title.toLowerCase().slice(0, -1)} associated with this trust shareholder.`} />
      <section className="mt-7 rounded-2xl border border-slate-200 bg-slate-50/60 p-5">
...
function OwnershipScreen({ subject, updateApplication, openOwner }: { subject: ApplicationSubject; updateApplication: (patch: Partial<ShareholderApplication>) => void; openOwner: (id: string) => void }) {
  ...
  return (
    <div className="animate-[fadeUp_.25s_ease-out]">
      <ScreenHeading eyebrow="Interconnection" title={subject.type === "trust" ? "Beneficial ownership structure" : "Shareholder ownership structure"} description={`Add the owners of ${subject.name}. Every disclosed layer must total exactly 100%, with recursive applications wherever a corporate entity or trust holds 25% or more.`} />
      ...
      <section className="mt-7 rounded-2xl border border-slate-200 bg-slate-50/65 p-5 sm:p-6">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><h3 className="text-base font-semibold text-slate-950">Add an owner</h3><p className="mt-1 text-sm text-slate-500">Record ownership at this layer.</p></div><span className={`w-fit rounded-full px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.06em] ${layerComplete ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>Layer total {total.toFixed(2).replace(/\.00$/, "")}%</span></div>
        ...
        <button type="button" onClick={addOwner} className="mt-5 inline-flex h-11 items-center gap-2 rounded-xl bg-[#003478] px-4 text-xs font-semibold text-white hover:bg-[#002b63]"><Plus className="h-4 w-4" /> Add owner</button>
      </section>
          ...
          return (
            <div key={owner.id} className="rounded-2xl border border-slate-200 bg-white p-4">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
                ...
                <div className="flex items-center gap-2"><button type="button" onClick={() => openOwner(owner.id)} className="inline-flex h-9 items-center gap-2 rounded-xl border border-[#003478]/20 bg-white px-3 text-xs font-semibold text-[#003478] hover:bg-[#f3f7fb]">{applicationComplete ? "Review application" : "Fill application"}<ChevronRight className="h-3.5 w-3.5" /></button><button type="button" onClick={() => updateApplication({ ownershipInterests: owners.filter((saved) => saved.id !== owner.id) })} aria-label={`Remove ${owner.name}`} className="grid h-9 w-9 place-items-center rounded-xl text-slate-400 hover:bg-red-50 hover:text-red-600"><Trash2 className="h-4 w-4" /></button></div>
              </div>
            </div>
          );
...
function IdentityScreen({ subject, updateApplication, onOpenDocument }: { subject: ApplicationSubject; updateApplication: (patch: Partial<ShareholderApplication>) => void; onOpenDocument: (document: UploadedDocument, label: string) => void }) {
  const updateSelfie = (file?: File) => updateApplication({ documents: { ...subject.application.documents, selfie: documentFromFile(file) } });
  return <div className="animate-[fadeUp_.25s_ease-out]"><ScreenHeading eyebrow="Identity" title="Identity check" description="Add a clear current selfie for the individual shareholder’s identity verification." /><div className="mt-7"><SelfieUpload value={subject.application.documents.selfie} onChange={updateSelfie} onPreview={onOpenDocument} /></div></div>;
}
...
function ReviewScreen({ subject, steps, updateApplication }: { subject: ApplicationSubject; steps: ScreenDefinition[]; updateApplication: (patch: Partial<ShareholderApplication>) => void }) {
  const completed = isShareholderApplicationComplete(subject);
  const reviewableSteps = steps.filter((step) => step.id !== "review");
  return (
    <div className="animate-[fadeUp_.25s_ease-out]">
      <ScreenHeading eyebrow="Final check" title="Review this shareholder application" description="Confirm every relevant section before returning to the main company application." />
