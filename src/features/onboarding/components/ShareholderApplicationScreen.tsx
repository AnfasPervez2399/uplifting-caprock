...
import {
  createEmptyShareholderOwner,
  createEmptyShareholderRelatedParty,
} from "../initialState";
import {
...
type NestedRole = "shareholder" | "beneficiary" | "trustee";
...
const nestedListLabel = (role: NestedRole) =>
  role === "beneficiary"
    ? "beneficiaries"
    : role === "trustee"
      ? "trustees"
...
const nestedTitle = (role: NestedRole) =>
  role === "beneficiary"
    ? "Beneficiary"
    : role === "trustee"
      ? "Trustee"
...
const hasIndividualProofDocuments = (subject: ApplicationSubject) =>
  Boolean(
    subject.application.documents.photoIdentity &&
    subject.application.documents.addressEvidence,
  );

const getApplicationStarted = (subject: ApplicationSubject) =>
  Boolean(
...
const getApplicationLabel = (
  ...
  _role: NestedRole = "shareholder",
) =>
  isShareholderApplicationComplete(subject)
    ? "Complete"
    : getApplicationStarted(subject)
      ? "In progress"
      : "Not started";
  ...
  return (
    <div className="fixed inset-0 z-[120] overflow-y-auto bg-[#f7f9fb]">
      <ApplicationFlow
        ...
        ancestry={
          role === "beneficiary"
            ? ["Trust application", "Beneficiaries"]
            : role === "trustee"
              ? ["Trust application", "Trustees"]
              : ["Company application", "Shareholders"]
        }
        depth={1}
        role={role}
      />
...
function isScreenComplete(
  ...
  role: NestedRole = "shareholder",
) {
  if (screen === "profile")
    return isShareholderApplicationProfileComplete(subject);
  if (screen === "business") return isShareholderBusinessComplete(subject);
  if (screen === "directors")
    return isShareholderRelatedPartiesComplete(subject);
  if (screen === "trustees") {
    return (
      subject.application.trustees.length > 0 &&
      subject.application.trustees.every(isRelatedPartyComplete)
    );
  }
  if (screen === "beneficiaries") {
    return (
      subject.application.beneficiaries.length > 0 &&
      subject.application.beneficiaries.every(isRelatedPartyComplete)
    );
  }
  if (screen === "ownership") return isShareholderOwnershipComplete(subject);
  if (screen === "identity")
    return Boolean(subject.application.documents.selfie);
  if (screen === "documents") {
    return subject.type === "individual"
      ? hasIndividualProofDocuments(subject)
      : isShareholderDocumentsComplete(subject);
  }
  if (screen === "signature") return isShareholderSignatureComplete(subject);
  return getApplicationLabel(subject, role) === "Complete";
}
...
function ApplicationFlow<T extends ApplicationSubject>({
  ...
  const steps = useMemo(() => getScreens(subject, role), [role, subject.type]);
  const [screen, setScreen] = useState<ApplicationScreen>("profile");
  const [activeOwnerPath, setActiveOwnerPath] =
    useState<string[]>(initialOwnerPath);
  const [screenError, setScreenError] = useState("");
  const activeOwner = subject.application.ownershipInterests.find(
    (owner) => owner.id === activeOwnerPath[0],
  );
      ...
      <ApplicationFlow
        ...
        onChange={(nextOwner) => {
          onChange({
            ...
            application: {
              ...subject.application,
              ownershipInterests: subject.application.ownershipInterests.map(
                (owner) => (owner.id === nextOwner.id ? nextOwner : owner),
              ),
            },
          });
        }}
  ...
  const currentIndex = Math.max(
    0,
    steps.findIndex((step) => step.id === screen),
  );
  ...
  const goForward = () => {
    if (!isScreenComplete(subject, screen, role)) {
      const messages: Partial<Record<ApplicationScreen, string>> = {
        ...
        business:
          "Complete the principal business address, activity, source of funds and intended transaction details.",
        directors:
          "Add at least one complete director with a valid email address and phone number.",
        ...
        signature:
          "Complete the authorised signatory’s name, valid email, phone number and adult date of birth.",
      };
      setScreenError(
        messages[screen] || "Complete this section before continuing.",
      );
      return;
    }
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-[1440px] items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
          <button
            type="button"
            onClick={onBack}
            className="inline-flex h-10 items-center gap-2 rounded-xl px-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100 hover:text-[#003478]"
          >
            <ArrowLeft className="h-4 w-4" />
            <span className="hidden sm:inline">
              {depth === 1
                ? `Back to ${nestedListLabel(role)}`
                : "Previous ownership layer"}
            </span>
            <span className="sm:hidden">Back</span>
          </button>
          <div className="min-w-0 text-center">
            <p className="truncate text-sm font-semibold text-slate-950">
              {getTypeLabel(subject)} {depth === 1 ? role : "owner"} application
            </p>
            <p className="mt-0.5 text-[10px] font-bold uppercase tracking-[0.1em] text-[#003478]">
              {role === "trustee" && depth === 1
                ? "Company application"
                : `Ownership layer ${depth}`}
            </p>
          </div>
          <button
            type="button"
      ...
      <div className="mx-auto max-w-[1440px] px-4 py-5 sm:px-6 sm:py-8 lg:px-8">
            ...
            <div className="flex min-w-0 items-center gap-4">
              ...
              <div className="min-w-0">
                <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#003478]">
                  {role === "trustee" && depth === 1
                    ? "Corporate trustee"
                    : `${getTypeLabel(subject)} · ${subject.percentage}% ownership`}
                </p>
                <h1 className="mt-1 truncate text-xl font-semibold tracking-[-0.025em] text-slate-950 sm:text-2xl">
                  {subject.name}
                </h1>
              </div>
            </div>
            <span
              className={`inline-flex w-fit items-center gap-1.5 rounded-full px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.07em] ${getApplicationLabel(subject, role) === "Complete" ? "bg-emerald-50 text-emerald-700" : "bg-[#f3f6f9] text-slate-600"}`}
            >
              {getApplicationLabel(subject, role) === "Complete" ? (
                <CheckCircle2 className="h-3.5 w-3.5" />
              ) : (
                <Info className="h-3.5 w-3.5" />
              )}
              {getApplicationLabel(subject, role)}
            </span>
          </div>
        </div>
        ...
        <div className="mb-6 overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-sm">
          ...
          <nav
            ...
            aria-label={`${nestedTitle(role)} application sections`}
          >
            <div className="flex min-w-max items-start">
              {steps.map((item, index) => {
                const Icon = item.icon;
                const active = item.id === screen;
                const complete = isScreenComplete(subject, item.id, role);
                return (
                  <div key={item.id} className="flex items-start">
                    ...
                    >
                      ...
                      >
                        ...
                        {active ? (
                          <span className="absolute -bottom-1 h-2 w-2 rotate-45 border-b border-r border-[#003478] bg-[#dce7f2]" />
                        ) : null}
                      </span>
                      <span
                    ...
                    {index < steps.length - 1 ? (
                      <div className="relative mt-[21px] h-px w-8 bg-slate-200 sm:w-12">
                        <span
                          className={`absolute inset-y-0 left-0 transition-all duration-500 ${index < currentIndex || complete ? "w-full bg-[#003478]" : "w-0 bg-[#003478]"}`}
                        />
                      </div>
                    ) : null}
                  </div>
        ...
        <main
          key={`${subject.id}-${screen}`}
          className="shareholder-flow-panel min-w-0 rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm sm:p-8"
        >
          {screen === "profile" ? (
            <ProfileScreen
              subject={subject}
          ...
          {screen === "trustees" || screen === "beneficiaries" ? (
            <TrustPartiesScreen
              kind={screen}
              subject={subject}
              updateApplication={updateApplication}
          ...
          {screen === "ownership" ? (
            <OwnershipScreen
              subject={subject}
              updateApplication={updateApplication}
              openOwnerPath={setActiveOwnerPath}
              role={role}
            />
          ) : null}
          {screen === "identity" ? (
          ...
          <div className="mt-8 flex flex-col-reverse gap-3 border-t border-slate-100 pt-6 sm:flex-row sm:items-center sm:justify-between">
            ...
            {screen === "review" ? (
              <button
                type="button"
                onClick={onBack}
                disabled={getApplicationLabel(subject, role) !== "Complete"}
                className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#003478] px-5 text-sm font-semibold text-white transition hover:bg-[#002b63] disabled:cursor-not-allowed disabled:bg-slate-300"
              >
              ...
              <button
                type="button"
                onClick={goForward}
                className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#003478] px-5 text-sm font-semibold text-white transition hover:bg-[#002b63]"
              >
                Continue · {steps[currentIndex + 1]?.label}
    ...
    <div className="animate-[fadeUp_.25s_ease-out]">
      ...
      <div className="mt-7 grid gap-5 sm:grid-cols-2">
        <Field
          ...
          htmlFor={`owner-name-${subject.id}`}
        >
          <input
            id={`owner-name-${subject.id}`}
            value={subject.name}
            onChange={(event) => updateSubject({ name: event.target.value })}
            placeholder="Enter full legal name"
            className={inputClass()}
          />
        </Field>
        <Field
          label={subject.type === "trust" ? "Trust contact email" : "Email"}
          htmlFor={`owner-email-${subject.id}`}
        >
          <input
            id={`owner-email-${subject.id}`}
            name={`shareholder-email-${subject.id}`}
            type="email"
            inputMode="email"
            autoComplete="email"
            spellCheck={false}
        ...
        <Field label="Phone number" htmlFor={`owner-phone-${subject.id}`}>
          <input
            id={`owner-phone-${subject.id}`}
            type="tel"
            value={subject.phone}
            onChange={(event) => updateSubject({ phone: event.target.value })}
        ...
        <Field label="Country" htmlFor={`owner-country-${subject.id}`}>
          <CustomSelect
            id={`owner-country-${subject.id}`}
            value={subject.application.country}
            onChange={(country) => updateApplication({ country })}
        ...
        {subject.type === "corporate" ? (
          <Field
            ...
            htmlFor={`owner-company-type-${subject.id}`}
          >
            <CustomSelect
              id={`owner-company-type-${subject.id}`}
              value={subject.companyType}
              onChange={(companyType) =>
                updateSubject({
                  companyType: companyType as CorporateEntityType,
                })
              }
        ...
        {subject.type === "trust" ? (
          <Field
            ...
            htmlFor={`owner-trust-type-${subject.id}`}
          >
            <CustomSelect
              id={`owner-trust-type-${subject.id}`}
              value={subject.trustType}
              onChange={(trustType) =>
                updateSubject({ trustType: trustType as ShareholderTrustType })
              }
        ...
        {subject.type === "individual" ? (
          <div className="sm:col-span-2">
            <Field label="Date of birth" htmlFor={`owner-dob-${subject.id}`}>
              <DatePicker
                ...
                value={subject.application.dateOfBirth}
                onChange={(dateOfBirth) => updateApplication({ dateOfBirth })}
                minYear={new Date().getFullYear() - 110}
                maxYear={new Date().getFullYear()}
                conditions={[minimumAgeCondition(18)]}
                helperText="The individual must be at least 18 years old."
              />
            </Field>
          </div>
        ) : (
          <Field
            label={
              subject.type === "trust"
                ? "Trust registration or reference number"
                : "Company registration number"
            }
            htmlFor={`owner-registration-${subject.id}`}
          >
            <input
              id={`owner-registration-${subject.id}`}
              value={subject.application.registrationNumber}
              onChange={(event) =>
                updateApplication({ registrationNumber: event.target.value })
              }
        ...
        <div className="sm:col-span-2">
          <Field
            ...
            htmlFor={`owner-address-${subject.id}`}
          >
            <textarea
              id={`owner-address-${subject.id}`}
              value={subject.application.address}
              onChange={(event) =>
                updateApplication({ address: event.target.value })
              }
...
function BusinessScreen({
  ...
  const updateBusiness = (patch: Partial<ShareholderApplication["business"]>) =>
    updateApplication({ business: { ...business, ...patch } });
  return (
    <div className="animate-[fadeUp_.25s_ease-out]">
      <ScreenHeading
        eyebrow="Business"
      ...
      <div className="mt-7 grid gap-5 sm:grid-cols-2">
        <div className="sm:col-span-2">
          ...
          >
            <textarea
              id={`business-address-${subject.id}`}
              value={business.principalBusinessAddress}
              onChange={(event) =>
                updateBusiness({ principalBusinessAddress: event.target.value })
              }
              placeholder="Full principal place of business"
              className={textareaClass()}
            />
          </Field>
        </div>
        <div className="sm:col-span-2">
          ...
          >
            <textarea
              id={`business-activity-${subject.id}`}
              value={business.businessActivity}
              onChange={(event) =>
                updateBusiness({ businessActivity: event.target.value })
              }
              placeholder="Describe the principal business activity"
              className={textareaClass()}
            />
          </Field>
        </div>
        <div className="sm:col-span-2">
          ...
          >
            <textarea
              id={`business-funds-${subject.id}`}
              value={business.sourceOfFunds}
              onChange={(event) =>
                updateBusiness({ sourceOfFunds: event.target.value })
              }
              placeholder="Explain how the investment funds were generated"
              className={textareaClass()}
            />
          </Field>
        </div>
        <div className="sm:col-span-2">
          ...
          >
            <textarea
              id={`business-transactions-${subject.id}`}
              value={business.intendedTransactions}
              onChange={(event) =>
                updateBusiness({ intendedTransactions: event.target.value })
              }
              placeholder="Describe expected transaction types and frequency"
              className={textareaClass()}
            />
          </Field>
        </div>
...
function DirectorsScreen({
  ...
  const [draft, setDraft] = useState<CompanyDirector>(() => ({
    id: `shareholder-director-${Date.now()}`,
    name: "",
    email: "",
    phone: "",
  }));
  ...
  return (
    <div className="animate-[fadeUp_.25s_ease-out]">
      <ScreenHeading
        ...
        title="Directors"
        description="Add every director or authorised corporate party associated with this shareholder company."
      />
      <section className="mt-7 rounded-2xl border border-slate-200 bg-slate-50/60 p-5">
        <div className="grid gap-4 sm:grid-cols-3">
          <Field label="Full name" htmlFor={`director-name-${subject.id}`}>
            <input
              id={`director-name-${subject.id}`}
              value={draft.name}
              onChange={(event) =>
                setDraft((current) => ({
                  ...current,
          ...
          <Field label="Email" htmlFor={`director-email-${subject.id}`}>
            <input
              id={`director-email-${subject.id}`}
              type="email"
              value={draft.email}
              onChange={(event) =>
                setDraft((current) => ({
          ...
          <Field label="Phone number" htmlFor={`director-phone-${subject.id}`}>
            <input
              id={`director-phone-${subject.id}`}
              value={draft.phone}
              onChange={(event) =>
                setDraft((current) => ({
                  ...current,
        ...
        <button
          type="button"
          onClick={add}
          className="mt-4 inline-flex h-10 items-center gap-2 rounded-xl bg-[#003478] px-4 text-xs font-semibold text-white hover:bg-[#002b63]"
        >
      ...
      <SavedPartyList
        ...
        onRemove={(id) =>
          updateApplication({
            directors: directors.filter((party) => party.id !== id),
          })
        }
      />
    </div>
...
function TrustPartiesScreen({
  kind,
  subject,
  updateApplication,
}: {
  kind: "trustees" | "beneficiaries";
  subject: ApplicationSubject;
  updateApplication: (patch: Partial<ShareholderApplication>) => void;
}) {
  const parties = subject.application[kind];
  const [draft, setDraft] = useState<ShareholderRelatedParty>(() =>
    createEmptyShareholderRelatedParty(kind.slice(0, -1)),
  );
  const [error, setError] = useState("");
  ...
  const add = () => {
    if (!isRelatedPartyComplete(draft)) {
      setError(
        `Complete the ${title.toLowerCase().slice(0, -1)} type, structure where applicable, name, valid email and phone number.`,
      );
      return;
    }
  ...
  return (
    <div className="animate-[fadeUp_.25s_ease-out]">
      <ScreenHeading
        eyebrow="Related parties"
        title={title}
        description={`Add every ${title.toLowerCase().slice(0, -1)} associated with this trust application.`}
      />
      <section className="mt-7 rounded-2xl border border-slate-200 bg-slate-50/60 p-5">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <Field
            label={`${title.slice(0, -1)} type`}
            htmlFor={`${kind}-type-${subject.id}`}
          >
            <CustomSelect
              id={`${kind}-type-${subject.id}`}
              value={draft.type}
              onChange={(type) =>
                setDraft((current) => ({
                  ...current,
                  type: type as PartyType,
                  companyType: type === "corporate" ? current.companyType : "",
                }))
              }
          ...
          {draft.type === "corporate" ? (
            <Field
              label="Company structure"
              htmlFor={`${kind}-structure-${subject.id}`}
            >
              <CustomSelect
                id={`${kind}-structure-${subject.id}`}
          ...
          <Field label="Full legal name" htmlFor={`${kind}-name-${subject.id}`}>
            <input
              id={`${kind}-name-${subject.id}`}
              value={draft.name}
              onChange={(event) =>
                setDraft((current) => ({
                  ...current,
          ...
          <Field label="Email" htmlFor={`${kind}-email-${subject.id}`}>
            <input
              id={`${kind}-email-${subject.id}`}
              type="email"
              value={draft.email}
              onChange={(event) =>
                setDraft((current) => ({
        ...
        <button
          ...
          className="mt-4 inline-flex h-10 items-center gap-2 rounded-xl bg-[#003478] px-4 text-xs font-semibold text-white hover:bg-[#002b63]"
        >
          <Plus className="h-4 w-4" /> Add {title.toLowerCase().slice(0, -1)}
        </button>
      </section>
      <SavedPartyList
        parties={parties}
        ...
        onRemove={(id) =>
          updateApplication({
            [kind]: parties.filter((party) => party.id !== id),
          })
        }
      />
    </div>
...
function SavedPartyList({
  parties,
  empty,
  onRemove,
}: {
  parties: Array<CompanyDirector | ShareholderRelatedParty>;
  empty: string;
...
function OwnershipScreen({
  ...
  subject: ApplicationSubject;
  updateApplication: (patch: Partial<ShareholderApplication>) => void;
  openOwnerPath: (path: string[]) => void;
  role: NestedRole;
}) {
  const [draft, setDraft] = useState<ShareholderOwner>(
    createEmptyShareholderOwner,
  );
  ...
  return (
    <div className="animate-[fadeUp_.25s_ease-out]">
      <ScreenHeading
        ...
        title={
          subject.type === "trust" || role === "beneficiary"
            ? "Beneficial ownership structure"
            : "Shareholder ownership structure"
        }
        description={`Add the owners of ${subject.name}. Every disclosed layer must total exactly 100%, with recursive applications wherever a corporate entity or trust holds 25% or more.`}
      />
      <div className="mt-6 flex items-start gap-3 rounded-2xl border border-[rgba(0,52,120,0.16)] bg-[#f3f7fb] p-4 sm:p-5">
        ...
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-slate-600">
            Continue recursively until an individual ultimate beneficial owner
            is identified. A corporate entity or trust below 25% does not
            require another ownership cascade.
          </p>
        </div>
      </div>
      <section className="mt-7 rounded-2xl border border-slate-200 bg-slate-50/65 p-5 sm:p-6">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <Field label="Owner type" htmlFor={`nested-owner-type-${subject.id}`}>
            <CustomSelect
              id={`nested-owner-type-${subject.id}`}
              value={draft.type}
              onChange={(value) =>
                setDraft((current) => ({
                  ...current,
                  type: value as PartyType,
                  companyType: value === "corporate" ? current.companyType : "",
          ...
          {draft.type === "corporate" ? (
            <Field
              label="Company structure"
              htmlFor={`nested-company-type-${subject.id}`}
            >
              <CustomSelect
                id={`nested-company-type-${subject.id}`}
                value={draft.companyType}
                onChange={(companyType) =>
                  setDraft((current) => ({
                    ...current,
          ...
          {draft.type === "trust" ? (
            <Field
              label="Trust structure"
              htmlFor={`nested-trust-type-${subject.id}`}
            >
              <CustomSelect
                id={`nested-trust-type-${subject.id}`}
                value={draft.trustType}
                onChange={(trustType) =>
                  setDraft((current) => ({
                    ...current,
                    trustType: trustType as ShareholderTrustType,
                  }))
                }
                options={SHAREHOLDER_TRUST_TYPE_OPTIONS}
                placeholder="Select structure"
          ...
          <Field label="Full legal name" htmlFor={`nested-name-${subject.id}`}>
            <input
              id={`nested-name-${subject.id}`}
              value={draft.name}
              onChange={(event) =>
                setDraft((current) => ({
                  ...current,
          ...
          <Field
            ...
            htmlFor={`nested-email-${subject.id}`}
          >
            <input
              id={`nested-email-${subject.id}`}
              name={`nested-owner-email-${subject.id}`}
              type="email"
              inputMode="email"
              autoComplete="email"
              spellCheck={false}
      ...
      {layerComplete ? (
        <div className="mt-7">
          <NestedOwnershipGraph
            subject={subject}
            onOpenPath={openOwnerPath}
            context={role === "beneficiary" ? "beneficiaries" : "shareholders"}
          />
        </div>
      ) : owners.length ? (
        <div className="mt-7 flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-800">
          ...
          <p>
            ...
            interconnection graph and owner application actions.
          </p>
        </div>
      ) : null}
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      onClick={() =>
                        updateApplication({
                          ownershipInterests: owners.filter(
                            (saved) => saved.id !== owner.id,
                          ),
                        })
                      }
  ...
  return (
    <div className="animate-[fadeUp_.25s_ease-out]">
      <ScreenHeading
        ...
        description={`Add a clear current selfie for the individual ${role}’s identity verification.`}
      />
      <div className="mt-7">
        <SelfieUpload
          value={subject.application.documents.selfie}
...
function DocumentsScreen({
  ...
  const updateDocument = (
    key: keyof ShareholderApplicationDocuments,
    file?: File,
  ) =>
    updateApplication({
      documents: {
        ...subject.application.documents,
        [key]: documentFromFile(file),
      },
    });
  ...
  return (
    <div className="animate-[fadeUp_.25s_ease-out]">
      <ScreenHeading
        eyebrow="Documents"
        title="Upload proof"
        description="Upload current, legible evidence. Select any uploaded file to open it in the document viewer."
      />
      <div className="mt-7 grid gap-4 sm:grid-cols-2">
        {subject.type === "individual" ? (
          <>
            <DocumentUpload
              id={`photo-id-${subject.id}`}
              title="Photo identity"
              description="Passport, driving licence or government-issued photo identification."
              value={documents.photoIdentity}
              onChange={(file) => updateDocument("photoIdentity", file)}
              onPreview={onOpenDocument}
            />
            <DocumentUpload
              id={`address-evidence-${subject.id}`}
              title="Residential address evidence"
              description="Recent utility bill, tax document or other accepted address evidence."
              value={documents.addressEvidence}
              onChange={(file) => updateDocument("addressEvidence", file)}
              onPreview={onOpenDocument}
            />
          </>
        ) : null}
        {subject.type === "corporate" ? (
          <>
            <DocumentUpload
              id={`entity-registration-${subject.id}`}
              title="Registration evidence"
              description="Certificate of registration or current company extract."
              value={documents.entityRegistration}
              onChange={(file) => updateDocument("entityRegistration", file)}
              onPreview={onOpenDocument}
            />
            <DocumentUpload
              id={`ownership-chart-${subject.id}`}
              title="Ownership chart"
              description="Current chart showing all shareholders and ownership percentages."
              value={documents.ownershipChart}
              onChange={(file) => updateDocument("ownershipChart", file)}
              onPreview={onOpenDocument}
            />
          </>
        ) : null}
        {subject.type === "trust" ? (
          <>
            <DocumentUpload
              id={`trust-deed-${subject.id}`}
              title="Trust deed"
              description="Current executed trust deed, including relevant schedules."
              value={documents.trustDeed}
              onChange={(file) => updateDocument("trustDeed", file)}
              onPreview={onOpenDocument}
            />
            <DocumentUpload
              id={`ownership-chart-${subject.id}`}
              title="Ownership or control chart"
              description="Current chart showing trustees, beneficiaries and controlling parties."
              value={documents.ownershipChart}
              onChange={(file) => updateDocument("ownershipChart", file)}
              onPreview={onOpenDocument}
            />
...
function SignatureScreen({
  ...
  const updateSignature = (
    patch: Partial<ShareholderApplication["signature"]>,
  ) => updateApplication({ signature: { ...signature, ...patch } });
  return (
    <div className="animate-[fadeUp_.25s_ease-out]">
      <ScreenHeading
      ...
      <div className="mt-7 grid gap-5 sm:grid-cols-2">
        ...
        <Field label="Email" htmlFor={`signature-email-${subject.id}`}>
          <input
            id={`signature-email-${subject.id}`}
            type="email"
        ...
        <Field label="Phone number" htmlFor={`signature-phone-${subject.id}`}>
          <input
            id={`signature-phone-${subject.id}`}
...
function ReviewScreen({
  ...
  subject: ApplicationSubject;
  steps: ScreenDefinition[];
  updateApplication: (patch: Partial<ShareholderApplication>) => void;
  role: NestedRole;
}) {
  const completed = getApplicationLabel(subject, role) === "Complete";
  const reviewableSteps = steps.filter((step) => step.id !== "review");
  return (
    <div className="animate-[fadeUp_.25s_ease-out]">
      <ScreenHeading
        eyebrow="Final check"
        title={`Review this ${role} application`}
        description={`Confirm every relevant section before returning to the main ${role === "shareholder" ? "company" : "trust"} application.`}
      />
      <div className="mt-7 grid gap-3 sm:grid-cols-2">
        {reviewableSteps.map((step) => (
          <StatusCard
            key={step.id}
            label={step.label}
            complete={isScreenComplete(subject, step.id, role)}
            detail={
              isScreenComplete(subject, step.id, role)
      ...
      <label
        className={`mt-6 flex cursor-pointer items-start gap-3 rounded-2xl border p-4 transition ${subject.application.declarationAccepted ? "border-[#003478]/25 bg-[#f3f7fb]" : "border-slate-200 bg-white hover:border-slate-300"}`}
      >
        <input
          type="checkbox"
          checked={subject.application.declarationAccepted}
          onChange={(event) =>
            updateApplication({ declarationAccepted: event.target.checked })
          }
          className="mt-0.5 h-4 w-4 cursor-pointer accent-[#003478]"
        />
        <span className="text-sm leading-6 text-slate-700">
          <span className="font-semibold text-slate-950">
            {nestedTitle(role)} declaration
            <RequiredIndicator />
          </span>
          <span className="mt-1 block">
            {role === "trustee"
              ? "I confirm that these corporate trustee details are accurate. Directors listed in this application are the trustee’s directors."
              : "I confirm that these details are accurate and that all required ownership connections have been disclosed through complete 100% layers to the individual ultimate beneficial owner."}
          </span>
        </span>
      </label>
      <div
        className={`mt-6 flex items-start gap-3 rounded-2xl border p-4 ${completed ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}
      >
        {completed ? (
        ...
        <div>
          <p
            className={`text-sm font-semibold ${completed ? "text-emerald-900" : "text-amber-900"}`}
          >
            {completed
              ? "Application complete"
              : "Application not yet complete"}
          </p>
          <p
            className={`mt-1 text-xs leading-5 ${completed ? "text-emerald-700" : "text-amber-700"}`}
          >
            {completed
              ? `You can now return to the ${nestedListLabel(role)} list.`
              : "Complete every outstanding section and accept the declaration."}
          </p>
        </div>
      </div>
    </div>
...
function StatusCard({
  ...
  return (
    <div
      className={`rounded-2xl border p-4 ${complete ? "border-emerald-200 bg-emerald-50/70" : "border-amber-200 bg-amber-50/70"}`}
    >
      <div className="flex items-center gap-2">
        {complete ? (
