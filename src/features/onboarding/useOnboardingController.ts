...
export function useOnboardingController() {
  ...
  const visibleSteps = useMemo(() => getStepsForApplication(applicationType), [applicationType]);
  ...
  const shareholdersComplete = !isCompany || isCompanyShareholdersComplete(form.company, form.shareholders);
  ...
  const addDirector = (director: CompanyDirector) => {
    setForm((current) => ({ ...current, directors: [...current.directors, director] }));
  ...
  const removeDirector = (id: string) => {
    setForm((current) => ({
      ...
      company: { ...current.company, defaultRecipientId: current.company.defaultRecipientId === id ? "" : current.company.defaultRecipientId },
    }));
  };

  const addShareholder = (shareholder: CompanyShareholder) => {
    setForm((current) => ({ ...current, shareholders: [...current.shareholders, shareholder] }));
    setErrors((current) => ({ ...current, shareholders: "", shareholderCount: "" }));
  };

  const removeShareholder = (id: string) => {
    setForm((current) => ({ ...current, shareholders: current.shareholders.filter((shareholder) => shareholder.id !== id) }));
  };
  ...
  const removeTrustParty = (kind: "trustees" | "beneficiaries", id: string) => {
    setForm((current) => ({
      ...
      trust: kind === "trustees" && current.trust.defaultRecipientId === id
  ...
  const validateStep = (stepId: StepId) => {
    ...
    if (stepId === "directors" && !directorsComplete) {
      if (!form.company.directorCount || Number(form.company.directorCount) !== form.directors.length) nextErrors.directorCount = "Enter a count that matches the saved directors.";
      if (!form.company.defaultRecipientId) nextErrors.defaultDirector = "Select a default communication recipient.";
      nextErrors.directors = "The declared director count must match complete saved directors, and one director must be the default recipient.";
    }
    if (stepId === "shareholders" && !shareholdersComplete) {
      if (!form.company.shareholderCount || Number(form.company.shareholderCount) !== form.shareholders.length) nextErrors.shareholderCount = "Enter a count that matches the saved shareholders.";
      nextErrors.shareholders = "The declared shareholder count must match complete saved shareholder records.";
    }
    if (stepId === "trustees" && !trusteesComplete) {
      if (!form.trust.trusteeCount || Number(form.trust.trusteeCount) !== form.trustees.length) nextErrors.trusteeCount = "Enter a count that matches the saved trustees.";
      if (!form.trust.defaultRecipientId) nextErrors.defaultTrustee = "Select a default communication recipient.";
      nextErrors.trustees = "The declared trustee count must match complete saved trustees, and one trustee must be the default recipient.";
    }
    if (stepId === "beneficiaries" && !beneficiariesComplete) {
      if (!form.trust.beneficiaryCount || Number(form.trust.beneficiaryCount) !== form.beneficiaries.length) nextErrors.beneficiaryCount = "Enter a count that matches the saved beneficiaries.";
      nextErrors.beneficiaries = "The declared beneficiary count must match complete saved beneficiary records.";
    }
