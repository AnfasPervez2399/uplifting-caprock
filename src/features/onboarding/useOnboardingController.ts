...
export function useOnboardingController() {
  ...
  const [loggedUserName] = useState(() => sessionStorage.getItem("caprockUserName") || displayNameFromEmail(loggedUserEmail));
  ...
  const applicationSteps = useMemo(
    () => getStepsForApplication(applicationType).filter((step) => step.id !== "shareholders" || isPrivateCompany),
  ...
  const shareholdersComplete = !isPrivateCompany || isCompanyShareholdersComplete(form.company, form.shareholders);
  ...
  const canConfirmBeneficiaries = hasCompletePercentageLayer(form.beneficiaries) &&
  ...
  const addShareholder = (shareholder: CompanyShareholder) => {
    const projectedTotal = shareholderPercentageTotal + (Number(shareholder.percentage) || 0);
    if (projectedTotal > 100.0001) {
      setErrors((current) => ({ ...current, shareholderTotal: "Total direct ownership cannot exceed 100%.", shareholders: "Adjust the ownership percentage before adding this shareholder." }));
  ...
  const addBeneficiary = (beneficiary: TrustBeneficiary) => {
    const projectedTotal = beneficiaryPercentageTotal + (Number(beneficiary.percentage) || 0);
    if (projectedTotal > 100.0001) {
      setErrors((current) => ({
        ...current,
        beneficiaryTotal: "Total direct beneficial interest cannot exceed 100%.",
        beneficiaries: "Adjust the beneficial-interest percentage before adding this beneficiary.",
      }));
      setNotice("Total direct beneficial interest must equal 100% and cannot exceed it.");
      return;
    }
    setForm((current) => {
    ...
    setErrors((current) => ({ ...current, beneficiaries: "", beneficiaryCount: "", beneficiaryTotal: "" }));
  };

  const updateBeneficiary = (beneficiary: TrustBeneficiary) => {
    setForm((current) => ({
      ...current,
      beneficiaries: current.beneficiaries.map((saved) => saved.id === beneficiary.id ? beneficiary : saved),
    }));
  ...
  const removeBeneficiary = (id: string) => {
    setForm((current) => {
      const beneficiaries = current.beneficiaries.filter((beneficiary) => beneficiary.id !== id);
      return {
        ...current,
        beneficiaries,
        trust: {
    ...
    setErrors((current) => ({ ...current, beneficiaries: "", beneficiaryTotal: "" }));
  };
  ...
  const confirmAllBeneficiaries = () => {
    if (!canConfirmBeneficiaries) {
      ...
      setErrors((current) => ({
        ...
        beneficiaryTotal: totalComplete ? "" : "Adjust the beneficial interests so the total equals exactly 100%.",
      }));
      setSuccessNotice("");
      setNotice(totalComplete
    ...
    setErrors((current) => ({ ...current, beneficiaries: "", beneficiaryTotal: "" }));
    setNotice("");
    setSuccessNotice("The 100% direct beneficial-interest structure is saved. Beneficiary applications and interconnection layers are now available.");
  };
  ...
  const addTrustParty = (_kind: "trustees", party: TrustParty) => {
    ...
    setErrors((current) => ({ ...current, trustees: "", trusteeCount: "", defaultTrustee: "" }));
  };
    ...
    setForm((current) => {
      ...
      return {
        ...
        trust: {
          ...
          defaultRecipientId: current.trust.defaultRecipientId === id ? "" : current.trust.defaultRecipientId,
        },
      };
    });
  };
  ...
  const validateStep = (stepId: StepId) => {
    ...
    if (stepId === "shareholders" && !shareholdersComplete) {
      ...
      if (!hasCompletePercentageLayer(form.shareholders)) {
        nextErrors.shareholderTotal = shareholderPercentageTotal > 100
    ...
    if (stepId === "beneficiaries" && !beneficiariesComplete) {
      ...
      if (!hasCompletePercentageLayer(form.beneficiaries)) {
        nextErrors.beneficiaryTotal = beneficiaryPercentageTotal > 100
        ...
        nextErrors.beneficiaryTotal = "Select Save all beneficiaries to confirm the complete beneficial-interest structure.";
      }
