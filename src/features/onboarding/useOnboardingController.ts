...
import {
  ...
  getRequiredEntityDocuments,
  getStepsForApplication,
} from "./config";
import {
  createEmptyBank,
  createEmptyForm,
...
export function useOnboardingController() {
  ...
  const [loggedUserName] = useState(
    () =>
      sessionStorage.getItem("caprockUserName") ||
      displayNameFromEmail(loggedUserEmail),
  );
  ...
  const isPrivateCompany = isCompany && form.company.companyType === "private";
  ...
  const businessComplete = useMemo(() => {
    if (isCompany)
      return isCompanyBusinessComplete(applicationType, form.company);
  ...
  const companyProfileComplete = isCompanyProfileComplete(
    applicationType,
    form.company,
  );
  const directorsComplete =
    !isCompany || isCompanyDirectorsComplete(form.company, form.directors);
  const shareholderPercentageTotal = percentageTotal(form.shareholders);
  const canConfirmShareholders =
    hasCompletePercentageLayer(form.shareholders) &&
    form.shareholders.every(isCompleteShareholder);
  const shareholdersComplete =
    !isPrivateCompany ||
    isCompanyShareholdersComplete(form.company, form.shareholders);
  const trustProfileComplete = isTrustProfileComplete(
    applicationType,
    form.trust,
  );
  const trusteesComplete =
    !isTrust || isTrusteesComplete(form.trust, form.trustees);
  const beneficiaryPercentageTotal = percentageTotal(form.beneficiaries);
  const canConfirmBeneficiaries =
    hasCompletePercentageLayer(form.beneficiaries) &&
    form.beneficiaries.every(isCompleteBeneficiary);
  const beneficiariesComplete =
    !isTrust || isBeneficiariesComplete(form.trust, form.beneficiaries);
  ...
  const requiredEntityDocuments = getRequiredEntityDocuments(
    applicationType,
    form.company.companyType,
  );
  const documentsComplete = isIndividual
    ? proofApplicantProfiles.every((applicant) =>
        hasCompleteApplicantProof(
          form.documents[applicant.key],
          applicant.country,
        ),
      )
    : requiredEntityDocuments.length > 0 &&
      requiredEntityDocuments.every((document) =>
        Boolean(form.entityDocuments[document.key]),
      );
  ...
  const handleLogout = () => {
    ...
    sessionStorage.removeItem("caprockUserName");
    sessionStorage.removeItem("caprockUserMobile");
  ...
  const updateCompany = <K extends keyof CompanyState>(
    key: K,
  ...
  const updateTrust = <K extends keyof TrustState>(
    key: K,
    value: TrustState[K],
  ) => {
    setForm((current) => ({
      ...current,
      trust: { ...current.trust, [key]: value },
    }));
    setErrors((current) => ({
      ...current,
      [key]: "",
  ...
  const removeDirector = (id: string) => {
    setForm((current) => {
      const directors = current.directors.filter(
      ...
      return {
        ...
        company: {
          ...
          directorCount: String(directors.length),
          defaultRecipientId:
            current.company.defaultRecipientId === id
              ? ""
              : current.company.defaultRecipientId,
        },
      };
    });
  };
  ...
  const addShareholder = (shareholder: CompanyShareholder) => {
    const projectedTotal =
      shareholderPercentageTotal + (Number(shareholder.percentage) || 0);
    if (projectedTotal > 100.0001) {
      setErrors((current) => ({
        ...current,
        shareholderTotal: "Total direct ownership cannot exceed 100%.",
        shareholders:
          "Adjust the ownership percentage before adding this shareholder.",
      }));
      setNotice("Total direct ownership must equal 100% and cannot exceed it.");
      return;
    }
    setForm((current) => {
  ...
  const updateShareholder = (shareholder: CompanyShareholder) => {
    setForm((current) => ({
      ...current,
      shareholders: current.shareholders.map((saved) =>
        saved.id === shareholder.id ? shareholder : saved,
      ),
    }));
  ...
  const removeShareholder = (id: string) => {
    setForm((current) => {
      const shareholders = current.shareholders.filter(
  ...
  const confirmAllShareholders = () => {
    if (!canConfirmShareholders) {
      ...
      setErrors((current) => ({
        ...
        shareholderTotal: totalComplete
          ? ""
          : "Adjust ownership so the total equals exactly 100%.",
      }));
      setSuccessNotice("");
      setNotice(
        totalComplete
    ...
    setNotice("");
    setSuccessNotice(
      "The 100% direct ownership structure is saved. Shareholder applications and interconnection layers are now available.",
    );
  };

  const addBeneficiary = (beneficiary: TrustBeneficiary) => {
    const projectedTotal =
      beneficiaryPercentageTotal + (Number(beneficiary.percentage) || 0);
    if (projectedTotal > 100.0001) {
      setErrors((current) => ({
        ...current,
        beneficiaryTotal:
          "Total direct beneficial interest cannot exceed 100%.",
        beneficiaries:
          "Adjust the beneficial-interest percentage before adding this beneficiary.",
      }));
      setNotice(
        "Total direct beneficial interest must equal 100% and cannot exceed it.",
      );
      return;
    }
    setForm((current) => {
      const beneficiaries = [...current.beneficiaries, beneficiary];
      return {
        ...current,
        beneficiaries,
        trust: {
  ...
  const updateBeneficiary = (beneficiary: TrustBeneficiary) => {
    setForm((current) => ({
      ...current,
      beneficiaries: current.beneficiaries.map((saved) =>
        saved.id === beneficiary.id ? beneficiary : saved,
      ),
    }));
    setErrors((current) => ({ ...current, beneficiaries: "" }));
  };

  const removeBeneficiary = (id: string) => {
    setForm((current) => {
      const beneficiaries = current.beneficiaries.filter(
  ...
  const confirmAllBeneficiaries = () => {
    if (!canConfirmBeneficiaries) {
      const totalComplete = hasCompletePercentageLayer(form.beneficiaries);
      setErrors((current) => ({
        ...current,
        beneficiaries: totalComplete
          ? "Complete every beneficiary record before saving the beneficial-interest structure."
          : "Beneficial interests must total exactly 100% before the structure can be saved.",
        beneficiaryTotal: totalComplete
          ? ""
          : "Adjust the beneficial interests so the total equals exactly 100%.",
      }));
      setSuccessNotice("");
      setNotice(
        totalComplete
          ? "Complete every beneficiary’s type and contact details before saving."
          : `The current beneficiary total is ${beneficiaryPercentageTotal.toFixed(2).replace(/\.00$/, "")}% and must equal 100%.`,
      );
      return;
    }
    setForm((current) => ({
      ...current,
      trust: { ...current.trust, beneficiariesConfirmed: true },
    }));
    setErrors((current) => ({
      ...current,
      beneficiaries: "",
    ...
    setNotice("");
    setSuccessNotice(
      "The 100% direct beneficial-interest structure is saved. Beneficiary applications and interconnection layers are now available.",
    );
  };
  ...
  const addTrustParty = (_kind: "trustees", party: TrustParty) => {
    setForm((current) => {
      ...
      return {
        ...current,
        trustees,
        trust: { ...current.trust, trusteeCount: String(trustees.length) },
      };
    });
    setErrors((current) => ({
      ...current,
  ...
  const removeTrustParty = (_kind: "trustees", id: string) => {
    setForm((current) => {
      const trustees = current.trustees.filter((party) => party.id !== id);
      return {
        ...current,
        trustees,
        trust: {
          ...current.trust,
          trusteeCount: String(trustees.length),
          defaultRecipientId:
            current.trust.defaultRecipientId === id
              ? ""
  ...
  const handleApplicationTypeChange = (value: string) => {
    ...
    if (nextType === form.personal.applicationType) return;

    const nextForm = createEmptyForm();
    nextForm.personal.applicationType = nextType;
    ...
    setLookupState("idle");
    setLookupVerifiedAt("");
    setBankDraft(createEmptyBank());
    setEditingBankId(null);
    setShowAdviserInvite(false);
    setAdviserDraft(emptyAdviserDraft);
    setAdviserError("");
    try {
      sessionStorage.removeItem("caprockOnboardingDraft");
    } catch {
  ...
  const validateStep = (stepId: StepId) => {
    ...
    if (stepId === "entity" && !companyProfileComplete) {
      if (!form.company.name.trim())
        nextErrors.companyName = "Enter the registered company name.";
      if (
        form.company.website.trim() &&
        !/^https?:\/\//i.test(form.company.website.trim()) &&
        !/^[a-z0-9.-]+\.[a-z]{2,}/i.test(form.company.website.trim())
      )
        nextErrors.companyWebsite = "Enter a valid website address.";
      if (!form.company.investmentCurrency)
        nextErrors.companyInvestmentCurrency = "Select an investment currency.";
      if (!form.company.expectedInvestment)
        nextErrors.companyExpectedInvestment =
          "Select the expected investment amount.";
      if (applicationType === "australian-company") {
        if (!form.company.registrationNumber.trim())
          nextErrors.companyRegistrationNumber =
            "Enter the company registration number.";
        if (!form.company.stateOrTerritory)
          nextErrors.companyState = "Select the state or territory.";
        if (!form.company.companyType)
          nextErrors.companyType = "Select the company type.";
        if (!form.company.incorporationDate)
          nextErrors.companyIncorporationDate = "Enter the incorporation date.";
      } else if (
        applicationType === "asic-non-australian-company" &&
        !form.company.arbn.trim()
      ) {
        nextErrors.companyArbn = "Enter the Australian Registered Body Number.";
      } else if (
        applicationType === "non-australian-company" &&
        !form.company.registrationNumber.trim()
      ) {
        nextErrors.companyRegistrationNumber =
          "Enter the company registration number.";
      }
    ...
    if (stepId === "trust" && !trustProfileComplete) {
      if (!form.trust.name.trim())
        nextErrors.trustName = "Enter the full name of the trust.";
      if (!form.trust.trusteeBusinessName.trim())
        nextErrors.trustBusinessName = "Enter the trustee business name.";
      if (!form.trust.establishedCountry)
        nextErrors.trustEstablishedCountry =
          "Select the establishment country.";
    ...
    if (stepId === "business" && isCompany && !businessComplete) {
      if (!form.company.registeredAddress.trim())
        nextErrors.companyRegisteredAddress = "Enter the registered address.";
      if (applicationType === "australian-company") {
        if (!form.company.registrationType)
          nextErrors.companyRegistrationType =
            "Select the ASIC registration type.";
        if (!form.company.acn.trim())
          nextErrors.companyAcn = "Enter the Australian Company Number.";
        if (!form.company.amlActivity.trim())
          nextErrors.companyAmlActivity =
            "Provide the compliance disclosure, or enter “None”.";
      } else {
        if (!form.company.companyType)
          nextErrors.companyType = "Select the company type.";
        if (!form.company.incorporationDate)
          nextErrors.companyIncorporationDate = "Enter the incorporation date.";
        if (!form.company.country)
          nextErrors.companyCountry = "Select the country of formation.";
        if (!form.company.principalAddress.trim())
          nextErrors.companyPrincipalAddress =
            "Enter the principal business address.";
        if (!form.company.usRegistered)
          nextErrors.companyUsRegistered = "Confirm U.S. registration status.";
        if (form.company.usRegistered === "yes" && !form.company.usTaxId.trim())
          nextErrors.companyUsTaxId = "Enter the U.S. Tax ID.";
        if (
          applicationType === "asic-non-australian-company" &&
          !form.company.registeredByRelevantBody
        )
          nextErrors.companyRegisteredByRelevantBody =
            "Confirm registration with the relevant body.";
        if (applicationType === "non-australian-company") {
          if (!form.company.identificationNumber.trim())
            nextErrors.companyIdentificationNumber =
              "Enter the registration-body identification number.";
          if (!form.company.domicileCountry)
            nextErrors.companyDomicileCountry = "Select the domicile country.";
        }
      }
      nextErrors.business =
        "Complete every required business, registration, address and tax field for this company type.";
    }
    if (stepId === "business" && isTrust && !businessComplete) {
      if (!form.trust.investmentCurrency)
        nextErrors.trustInvestmentCurrency = "Select an investment currency.";
      if (!form.trust.expectedInvestment)
        nextErrors.trustExpectedInvestment =
          "Select the expected investment amount.";
      if (!form.trust.establishedCountry)
        nextErrors.trustEstablishedCountry =
          "Select the establishment country.";
      if (!form.trust.settlorName.trim())
        nextErrors.trustSettlor = "Enter the settlor’s full name.";
      if (!form.trust.address.trim())
        nextErrors.trustAddress = "Enter the trust address.";
      if (!form.trust.applicantCountry)
        nextErrors.trustApplicantCountry = "Select the applicant country.";
      if (
        applicationType !== "regulated-trust" &&
        !form.trust.afsLicenseNumber.trim()
      )
        nextErrors.trustAfsLicense =
          "Enter the Australian Financial Services Licence number.";
      nextErrors.business =
        "Complete every required trust investment, settlor, address and licence field.";
    }
    if (stepId === "directors" && !directorsComplete) {
      if (!form.directors.length)
        nextErrors.directorCount = "Add at least one director or partner.";
      if (!form.company.defaultRecipientId)
        nextErrors.defaultDirector =
          "Select a default communication recipient.";
      nextErrors.directors =
        "Add at least one complete director or partner and select a default communication recipient.";
    }
    if (stepId === "shareholders" && !shareholdersComplete) {
      if (!form.shareholders.length)
        nextErrors.shareholderCount = "Add at least one shareholder.";
      if (!hasCompletePercentageLayer(form.shareholders)) {
        nextErrors.shareholderTotal =
          shareholderPercentageTotal > 100
            ? "Direct shareholder ownership cannot exceed 100%."
            : "Direct shareholder ownership must total exactly 100%.";
      } else if (!form.company.shareholdersConfirmed) {
        nextErrors.shareholderTotal =
          "Select Save all shareholders to confirm the complete ownership structure.";
      }
    ...
    if (stepId === "trustees" && !trusteesComplete) {
      if (!form.trustees.length)
        nextErrors.trusteeCount = "Add at least one trustee.";
    ...
    if (stepId === "beneficiaries" && !beneficiariesComplete) {
      if (!form.beneficiaries.length)
        nextErrors.beneficiaryCount = "Add at least one beneficiary.";
      if (!hasCompletePercentageLayer(form.beneficiaries)) {
        nextErrors.beneficiaryTotal =
          beneficiaryPercentageTotal > 100
            ? "Direct beneficial interests cannot exceed 100%."
            : "Direct beneficial interests must total exactly 100%.";
      } else if (!form.trust.beneficiariesConfirmed) {
        nextErrors.beneficiaryTotal =
          "Select Save all beneficiaries to confirm the complete beneficial-interest structure.";
      }
      nextErrors.beneficiaries = form.trust.beneficiariesConfirmed
        ? "Complete every required beneficiary application and each recursively disclosed 100% ownership layer."
        : "Complete the beneficiary records, bring direct beneficial interests to exactly 100%, then select Save all beneficiaries.";
    }
