...
const groups: Array<{
...
}> = [
  {
    category: "individual",
    title: "Individual",
    description: "Personal, joint and sole-trader structures",
    icon: CircleUserRound,
  },
  {
    category: "company",
    title: "Company",
    description: "Australian and non-Australian registered entities",
    icon: Building2,
  },
  {
    category: "trust",
    title: "Trust",
    description: "Regulated, custodian and non-custodian trusts",
    icon: ShieldCheck,
  },
];
...
export function ApplicationTypeStep({
  ...
  const { form, errors, sectionEyebrow, handleApplicationTypeChange } =
    controller;
      ...
      {selected ? (
        <div className="mt-7 flex items-center gap-3 rounded-2xl border border-[#c7d7e7] bg-[#f5f8fb] p-4 text-sm text-slate-700">
          ...
          <p>
            ...
            {
              APPLICATION_OPTIONS.find((option) => option.value === selected)
                ?.label
            }
