      ...
      <div className="space-y-8">
        <section className="rounded-2xl border border-slate-200 bg-slate-50/55 p-5 sm:p-6">
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <Field label="Caprock reference number" htmlFor="referenceNumber" required={false} hint="Assigned automatically when available.">
              <input
                id="referenceNumber"
            ...
            <Field label="Adviser reference number" htmlFor="advisorReferenceNumber" required={false} hint="Supplied by your adviser when applicable.">
              <input
        ...
        <section>
          <SubsectionHeading title="Main applicant" description="Use legal identity details exactly as they appear on official documents." />
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            <div className="sm:col-span-2 lg:col-span-3">
              <DocumentUpload
                ...
                onChange={(file) => updatePersonal("profilePicture", documentFromFile(file))}
            ...
            >
              <input
                ...
                onChange={(event) => updatePersonal("formerNames", event.target.value)}
            ...
            <div className="sm:col-span-2 lg:col-span-3">
              <Field label="Residential address" htmlFor="residentialAddress" error={errors.residentialAddress}>
                <textarea
                  id="residentialAddress"
                  value={form.personal.residentialAddress}
                  onChange={(event) => updatePersonal("residentialAddress", event.target.value)}
        ...
        {isJoint ? (
          <section className="rounded-2xl border border-[rgba(0,52,120,0.15)] bg-[rgba(0,52,120,0.025)] p-5 sm:p-6">
            ...
            {form.jointApplicants.length > 0 ? (
              <div className="mb-5 space-y-3">
                {form.jointApplicants.map((applicant, index) => (
                  <div key={applicant.id} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-3.5">
                    <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[#dce7f2] text-[#003478]">
                      <Users className="h-4 w-4" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-semibold text-slate-900">{formatApplicantName(applicant)}</p>
                      <p className="mt-0.5 text-xs text-slate-500">
                        Applicant {index + 1} · {applicant.method === "existing" ? `Existing Caprock client · ${applicant.applicantCountry}` : applicant.email}
                      </p>
                    </div>
            ...
            {showJointComposer ? (
              <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                <div className="mb-5 flex items-center justify-between gap-3">
                  <div>
                    <p className="text-sm font-semibold text-slate-950">Add a joint applicant</p>
                    <p className="mt-1 text-xs text-slate-500">Choose how this person will join the application.</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                ...
                <div className="mb-5 grid grid-cols-2 gap-2 rounded-xl bg-slate-100 p-1">
                  {(["existing", "new"] as JointMethod[]).map((method) => (
                    ...
                    >
                      {method === "existing" ? "Existing client" : "Invite new client"}
                    </button>
                  ))}
                </div>
                ...
                {jointDraft.method === "existing" ? (
                  <div>
                    <Field label="Caprock client ID" htmlFor="jointClientId" hint="Client IDs are verified before the applicant is added.">
                      <div className="flex flex-col gap-2 sm:flex-row">
                        <input
                          id="jointClientId"
                          value={jointDraft.clientId}
                          onChange={(event) => updateJointDraft("clientId", event.target.value)}
                        ...
                        <button
                          ...
                          disabled={lookupState === "loading" || lookupState === "found"}
                        ...
                        >
                          ...
                          {lookupState === "loading" ? "Checking…" : lookupState === "found" ? "Verified" : "Verify ID"}
                        </button>
                      </div>
                    </Field>
                    {lookupState === "found" ? (
                      <div role="status" className="mt-4 overflow-hidden rounded-2xl border border-[rgba(0,52,120,0.16)] bg-[rgba(0,52,120,0.035)]">
                        <div className="flex items-start gap-3 p-4 sm:p-5">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              <p className="text-sm font-semibold text-slate-950">Client ID verified</p>
                              <span className="rounded-full bg-white px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.08em] text-[#003478] ring-1 ring-[rgba(0,52,120,0.12)]">
                                Verified {lookupVerifiedAt ? `at ${lookupVerifiedAt}` : "just now"}
                              </span>
                            </div>
                            <p className="mt-1.5 text-xs leading-5 text-slate-600">
                              A matching Caprock client record was found. Identity and contact details will be securely linked when this applicant is added.
                            </p>
                            <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] font-medium text-slate-600">
                              ...
                              <span className="rounded-lg bg-white px-2.5 py-1.5 ring-1 ring-slate-200">
                                {jointDraft.applicantCountry || form.personal.applicantCountry || "Australia"}
                              </span>
                      ...
                      <p className="mt-2 text-xs font-medium text-red-600">Enter a valid client ID with at least five characters.</p>
                    ) : null}
                  </div>
                ) : (
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label="Full name" htmlFor="jointFullName">
                      <input
                        ...
                        onChange={(event) => updateJointDraft("firstName", event.target.value)}
                        autoComplete="name"
                    ...
                    <Field label="Email address" htmlFor="jointEmail">
                      <input
                        ...
                        onChange={(event) => updateJointDraft("email", event.target.value)}
                        autoComplete="email"
                    ...
                    {sharedAddress ? (
                      <div className="sm:col-span-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3.5 text-xs leading-5 text-slate-600">
                        This applicant will use the main applicant’s residential address.
                      </div>
                    ) : (
                      <div className="sm:col-span-2">
                        <Field label="Residential address" htmlFor="jointResidentialAddress">
                          <textarea
                            id="jointResidentialAddress"
                            value={jointDraft.residentialAddress}
                            onChange={(event) => updateJointDraft("residentialAddress", event.target.value)}
                            autoComplete="street-address"
                            placeholder="Street, suburb or city, state or region, postcode and country"
                ...
                {errors.jointApplicants ? <p className="mt-4 text-xs font-medium text-red-600">{errors.jointApplicants}</p> : null}
                <div className="mt-5 flex justify-end">
                  ...
                  >
                    ...
                    {jointDraft.method === "existing" ? "Add verified client" : "Send invitation"}
                  </button>
                </div>
              </div>
            ) : null}
            ...
            {errors.jointApplicants && !showJointComposer ? (
              <p className="mt-3 text-xs font-medium text-red-600">{errors.jointApplicants}</p>
            ) : null}
          </section>
        ) : null}

        <section>
          <SubsectionHeading title="Investment profile" description="Provide the investment currency and expected investment range." />
          <div className="grid gap-5 sm:grid-cols-2">
            <Field label="Investment currency" htmlFor="investmentCurrency" error={errors.investmentCurrency}>
              <CustomSelect
                id="investmentCurrency"
                value={form.personal.investmentCurrency}
                onChange={(value) => updatePersonal("investmentCurrency", value)}
                options={INVESTMENT_CURRENCY_OPTIONS}
                placeholder="Select currency"
                error={Boolean(errors.investmentCurrency)}
              />
            </Field>
            <Field label="Expected investment amount" htmlFor="expectedInvestment" error={errors.expectedInvestment}>
              <CustomSelect
                id="expectedInvestment"
                value={form.personal.expectedInvestment}
                onChange={(value) => updatePersonal("expectedInvestment", value)}
