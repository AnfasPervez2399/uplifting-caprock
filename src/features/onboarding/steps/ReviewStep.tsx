...
export function ReviewStep({
  ...
  const entityProofCount = requiredEntityDocuments.filter(
    (document) => form.entityDocuments[document.key],
  ).length;
  const individualProofCount = proofApplicantProfiles.reduce(
    (total, applicant) => {
  ...
  return (
    <div className="animate-[fadeUp_.35s_ease-out]">
      <SectionIntro
        ...
        description={
          submitted
            ? "Your submitted application remains available while the onboarding team completes its review."
            : "Check every structure-specific section. Use Edit to make changes before secure submission."
        }
        icon={BadgeCheck}
      />
      ...
      {submitted ? (
        <div
          role="status"
          className="mb-6 rounded-2xl border border-[rgba(0,52,120,0.18)] bg-[#f3f7fb] p-4 sm:p-5"
        >
          <div className="flex items-start gap-3.5">
            ...
            <div>
              ...
              <p className="mt-1.5 text-xs leading-5 text-slate-600">
                No further action is needed. We’ll contact you if more
        ...
        <div className="mb-6 flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
          ...
          Some sections are incomplete. Complete all required information before
          submitting.
        </div>
      ) : (
        <div className="mb-6 flex items-start gap-3 rounded-2xl border border-[rgba(0,52,120,0.14)] bg-[rgba(0,52,120,0.035)] p-4 text-sm leading-6 text-slate-700">
          ...
          All application sections are complete and ready for final
          declarations.
        </div>
      )}
        ...
        >
          <dl className="grid gap-x-6 gap-y-5 sm:grid-cols-2">
            ...
            <SummaryItem
              ...
              value={submitted ? "Under review" : "Draft"}
            />
          </dl>
          {form.adviserAccess ? (
          ...
          >
            ...
            {isJoint ? (
              <PartyRows
                ...
                rows={form.jointApplicants.map((applicant) => ({
                  id: applicant.id,
                  name: formatApplicantName(applicant),
                  detail: `${applicant.method === "existing" ? `Client ID ${applicant.clientId}` : applicant.email} · ${applicant.applicantCountry}`,
                }))}
              />
            ) : null}
          </ReviewSection>
          ...
          <>
            ...
            >
              <dl className="grid gap-x-6 gap-y-5 sm:grid-cols-2 lg:grid-cols-3">
                ...
                />
                <SummaryItem
                  label={isAsicForeign ? "ARBN" : "Registration number"}
                  value={
                    isAsicForeign
                      ? form.company.arbn
                      : form.company.registrationNumber
                  }
                />
                <SummaryItem
                  label="Website"
                  value={form.company.website || "Not provided"}
                />
                <SummaryItem
                  label="Company type"
                  value={form.company.companyType || "Not selected"}
                />
                <SummaryItem
                  label="Incorporation date"
                  value={form.company.incorporationDate || "Not provided"}
                />
                <SummaryItem
                  label="Investment profile"
                  value={`${form.company.investmentCurrency || "—"} · ${form.company.expectedInvestment || "—"}`}
                />
              </dl>
            </ReviewSection>
            <ReviewSection
            ...
            >
              <dl className="grid gap-x-6 gap-y-5 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <SummaryItem
                  ...
                  value={
                    isDomesticCompany
                      ? `${form.company.registrationType || "—"} · ACN ${form.company.acn || "—"}`
                ...
                {!isDomesticCompany ? (
                  <SummaryItem
                    ...
                    value={form.company.principalAddress}
                  />
                ) : null}
                {isDomesticCompany ? (
                  <SummaryItem
                    label="Compliance disclosure"
                    value={form.company.amlActivity}
                  />
                ) : (
                  <SummaryItem
                    ...
                    value={
                      form.company.usRegistered === "yes"
                        ? "Yes · Tax ID provided securely"
                        : form.company.usRegistered === "no"
                          ? "No"
                          : "Not selected"
                    }
                  />
            ...
            <ReviewSection
              title="M.D. & Owners"
              icon={UsersRound}
              onEdit={() => goToStep("directors")}
            >
              <PartyRows
                title={`${form.directors.length} saved director${form.directors.length === 1 ? "" : "s"}`}
                rows={form.directors.map((director) => ({
                  id: director.id,
                  name: director.name,
                  detail: `${director.email} · ${director.phone}${form.company.defaultRecipientId === director.id ? " · Default recipient" : ""}`,
                }))}
              />
            </ReviewSection>
            {form.company.companyType === "private" ? (
              <ReviewSection
                title="Shareholders"
                icon={UsersRound}
              ...
              >
                ...
                <PartyRows
                  title={`${form.shareholders.length} saved shareholder${form.shareholders.length === 1 ? "" : "s"}`}
                  rows={form.shareholders.map((shareholder) => ({
                    id: shareholder.id,
                    name: shareholder.name,
                    detail: `${shareholder.type || "—"} · ${shareholder.percentage || "—"}% · ${shareholder.email} · ${isShareholderApplicationComplete(shareholder) ? "Application complete" : requiresShareholderApplication(shareholder) ? "Required application incomplete" : "Application available"}`,
                  }))}
                />
              </ReviewSection>
            ) : null}
        ...
        {isTrust ? (
          <>
            <ReviewSection
              title="Trust Profile"
              icon={ShieldCheck}
              onEdit={() => goToStep("trust")}
            >
              <dl className="grid gap-x-6 gap-y-5 sm:grid-cols-2 lg:grid-cols-3">
                <SummaryItem label="Trust name" value={form.trust.name} />
                <SummaryItem
                  label="Trustee business name"
                  value={form.trust.trusteeBusinessName}
                />
                <SummaryItem
                  label="Establishment country"
                  value={form.trust.establishedCountry}
                />
              </dl>
            </ReviewSection>
            <ReviewSection
            ...
            >
              <dl className="grid gap-x-6 gap-y-5 sm:grid-cols-2 lg:grid-cols-3">
                <SummaryItem
                  label="Investment profile"
                  value={`${form.trust.investmentCurrency || "—"} · ${form.trust.expectedInvestment || "—"}`}
                />
                <SummaryItem label="Settlor" value={form.trust.settlorName} />
                <SummaryItem
                  label="Applicant country"
                  value={form.trust.applicantCountry}
                />
                <SummaryItem
                  label="AFSL number"
        ...
        {isSoleTrader ? (
          ...
          >
            <dl className="grid gap-x-6 gap-y-5 sm:grid-cols-2 lg:grid-cols-3">
              <SummaryItem
                label="Assessment"
                value={form.business.assessmentNature || "Not selected"}
              />
              <SummaryItem
                label="Business name"
                ...
                >
                  ...
                  {account.verificationDocument ? (
                    <button
                      ...
                      className="inline-flex w-fit items-center gap-1.5 rounded-full bg-[#dce7f2] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.08em] text-slate-800 hover:text-[#003478]"
                    >
        ...
        >
          <div className="mb-4 grid gap-3 sm:grid-cols-2">
            <CheckRow
              ...
              label={`${isIndividual ? individualProofCount : entityProofCount} proof file${(isIndividual ? individualProofCount : entityProofCount) === 1 ? "" : "s"} attached`}
            />
            <CheckRow
              checked={documentsComplete}
              label="Required proof set complete"
            />
          </div>
          {isIndividual ? (
            <IndividualProofSummary controller={controller} />
          ) : (
            <div className="space-y-2">
              {requiredEntityDocuments.map((requirement) => {
                const file = form.entityDocuments[requirement.key];
                return (
                  <button
                    key={requirement.key}
                    type="button"
                    disabled={!file}
                    onClick={() =>
                      file && openDocumentPreview(file, requirement.title)
                    }
                    className="flex w-full items-center gap-3 rounded-xl bg-slate-50 px-3.5 py-3 text-left transition enabled:hover:bg-slate-100 disabled:cursor-default"
                  >
                    <FileCheck2
                      className={`h-4 w-4 shrink-0 ${file ? "text-[#003478]" : "text-slate-300"}`}
                    />
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-slate-50/70 p-5 sm:p-6">
        ...
        <div className="mt-5 space-y-3">
          ...
          >
            I confirm that the information and documents supplied are complete,
          ...
          <Declaration
            ...
            onChange={(checked) =>
              setForm((current) => ({
                ...current,
                agreements: { ...current.agreements, consent: checked },
              }))
            }
          >
...
function IndividualProofSummary({
  controller,
}: {
  controller: OnboardingController;
}) {
  ...
  return (
    <div className="space-y-3">
      {proofApplicantProfiles.map((applicant) => {
        ...
        const second = PHOTO_ID_OPTIONS.find(
          (option) => option.value === documents.secondaryPhotoIdType,
        )?.label;
        const address = hasDrivingLicenceProof(documents)
        ...
        return (
          ...
          >
            ...
            />
            <SummaryItem label="Address evidence" value={address} />
            <SummaryItem
              label="CV or website"
              value={
...
function Declaration({
  ...
  return (
    <label
      className={`flex items-start gap-3 rounded-xl border border-slate-200 bg-white p-4 transition ${disabled ? "cursor-default" : "cursor-pointer hover:border-slate-300"}`}
    >
      <input
        type="checkbox"
        checked={checked}
