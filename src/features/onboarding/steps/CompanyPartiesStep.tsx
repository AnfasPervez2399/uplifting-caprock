...
export function CompanyShareholdersStep({ controller }: { controller: OnboardingController }) {
  ...
  if (isPublicCompany) {
    return (
      <div className="animate-[fadeUp_.35s_ease-out]">
        <SectionIntro eyebrow={sectionEyebrow("shareholders")} title="Shareholders" description="Shareholder details are not requested for a public company in this application flow." icon={UsersRound} />
      </div>
    );
  }
  ...
  return (
    <div className="animate-[fadeUp_.35s_ease-out]">
      ...
      <div className="space-y-8">
        <section className="overflow-hidden rounded-2xl border border-[rgba(0,52,120,0.17)] bg-[#f3f7fb]">
          <div className="flex items-start gap-4 p-5 sm:p-6">
            ...
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-slate-600">Every shareholder has its own application. When a corporate entity or trust holds 25% or more shares, identify that entity’s shareholders in the next ownership layer and continue recursively until the individual ultimate beneficial owner is identified.</p>
              <div className="mt-4 inline-flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-xs font-semibold text-[#003478] ring-1 ring-[rgba(0,52,120,0.12)]"><GitBranch className="h-4 w-4" /> Applications at or above the 25% threshold are required before you can continue.</div>
            </div>
          </div>
        </section>
        ...
        <section>
          <SubsectionHeading title={`Saved shareholders (${form.shareholders.length})`} description="Open each shareholder application. Corporate entities and trusts at or above 25% must be complete before this section can progress." />
          <div className="space-y-3">
            {form.shareholders.length ? form.shareholders.map((shareholder, index) => {
              ...
              return (
                <div key={shareholder.id} className={`rounded-2xl border bg-white p-4 transition ${required && !complete ? "border-amber-200" : "border-slate-200"}`}>
                  ...
                  {required && !complete ? <div className="mt-3 flex items-start gap-2 rounded-xl bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-800"><GitBranch className="mt-0.5 h-3.5 w-3.5 shrink-0" />This {typeLabel.toLowerCase()} owns at least 25%. Complete its profile and every required ownership layer.</div> : null}
                </div>
              );
