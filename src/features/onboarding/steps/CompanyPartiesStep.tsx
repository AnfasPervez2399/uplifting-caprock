...
import { Building2, CheckCircle2, ChevronRight, GitBranch, Info, Mail, Plus, ShieldCheck, Trash2, UserRound, UsersRound } from "lucide-react";
    ...
    <div className="animate-[fadeUp_.35s_ease-out]">
      ...
      <div className="space-y-8">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
          <SubsectionHeading title="Director declaration" description="The director or partner count is calculated automatically from the saved records above." />
          <div className="grid gap-5 sm:grid-cols-2">
            <Field label="Number of directors or partners" htmlFor="directorCount" error={errors.directorCount} hint="Calculated automatically from saved directors and partners.">
              <input id="directorCount" type="number" value={form.directors.length} readOnly aria-readonly="true" className={`${inputClass(Boolean(errors.directorCount))} cursor-not-allowed bg-slate-100/80 text-slate-700`} />
            </Field>
...
export function CompanyShareholdersStep({ controller }: { controller: OnboardingController }) {
  ...
  const shareholderStructureSaved = form.company.shareholdersConfirmed && Math.abs(shareholderPercentageTotal - 100) < 0.0001;
  ...
  const updateDraftPercentage = (value: string) => {
    if (value !== "" && Number(value) > remainingOwnership) {
      setDraftError(`Total ownership cannot exceed 100%. You can allocate up to ${remainingOwnershipLabel}% more.`);
      return;
    }
  ...
  const add = () => {
    if (shareholderPercentageTotal + (Number(draft.percentage) || 0) > 100.0001) {
      setDraftError(`Total ownership cannot exceed 100%. You can allocate up to ${remainingOwnershipLabel}% more.`);
      return;
    }
    if (!isCompleteShareholder(draft)) {
      setDraftError("Complete the shareholder type, structure where applicable, ownership percentage, full name, valid email and phone number.");
      return;
    }
  ...
  return (
    <div className="animate-[fadeUp_.35s_ease-out]">
      ...
      <SectionIntro eyebrow={sectionEyebrow("shareholders")} title="Shareholders" description="Record each individual, corporate entity or trust shareholder and its ownership percentage." icon={UsersRound} />
      <div className="space-y-8">
        <section className="overflow-hidden rounded-2xl border border-[rgba(0,52,120,0.17)] bg-[#f3f7fb]">
          <div className="flex items-start gap-4 p-5 sm:p-6">
            ...
            <div>
              <h2 className="text-base font-semibold text-slate-950">Please provide shareholder details for your company here.</h2>
              <p className="mt-2 text-sm leading-6 text-slate-600">First add every direct shareholder and bring the total ownership to exactly 100%. Save the shareholder structure to reveal the overall interconnection graph and each shareholder application.</p>
              <div className="mt-4 inline-flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-xs font-semibold text-[#003478] ring-1 ring-[rgba(0,52,120,0.12)]"><GitBranch className="h-4 w-4" /> After saving, complete applications at or above the 25% threshold and disclose ownership recursively.</div>
            </div>
          </div>
        </section>

        <section className="rounded-2xl border border-slate-200 bg-slate-50/55 p-5 sm:p-6">
          <SubsectionHeading title="Add a shareholder" description="Choose the correct shareholder structure so the application can create the appropriate ownership path." />
          <div className={`mb-5 flex flex-col gap-2 rounded-xl border px-4 py-3 text-xs sm:flex-row sm:items-center sm:justify-between ${Math.abs(shareholderPercentageTotal - 100) < 0.0001 ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-amber-200 bg-amber-50 text-amber-800"}`}>
            <span className="font-semibold">Total ownership must equal exactly 100%.</span>
            <span className="font-bold tabular-nums">Allocated {shareholderPercentageTotal.toFixed(2).replace(/\.00$/, "")}% · Remaining {remainingOwnershipLabel}%</span>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {draft.type === "trust" ? <Field label="Trust structure" htmlFor="shareholderTrustType"><CustomSelect id="shareholderTrustType" value={draft.trustType} onChange={(value) => setDraft((current) => ({ ...current, trustType: value as typeof current.trustType }))} options={SHAREHOLDER_TRUST_TYPE_OPTIONS} placeholder="Select trust structure" /></Field> : null}
            <Field label="Ownership percentage" htmlFor="shareholderPercentage" hint={`Maximum available: ${remainingOwnershipLabel}%`}><input id="shareholderPercentage" type="number" min="0.01" max={remainingOwnership} step="0.01" value={draft.percentage} onChange={(event) => updateDraftPercentage(event.target.value)} placeholder={remainingOwnership > 0 ? `Up to ${remainingOwnershipLabel}` : "100% allocated"} disabled={remainingOwnership <= 0} className={inputClass()} /></Field>
            <Field label="Full legal name" htmlFor="shareholderName"><input id="shareholderName" value={draft.name} onChange={(event) => setDraft((current) => ({ ...current, name: event.target.value }))} placeholder="Person, entity or trust name" className={inputClass()} /></Field>
            <Field label={draft.type === "trust" ? "Trust contact email" : "Email"} htmlFor="shareholderEmail"><input id="shareholderEmail" name="new-shareholder-email" type="email" inputMode="email" autoComplete="email" spellCheck={false} value={draft.email} onChange={(event) => { const email = event.currentTarget.value; setDraft((current) => ({ ...current, email })); }} placeholder={draft.type === "trust" ? "trust.contact@example.com" : "name@example.com"} className={inputClass()} /></Field>
          ...
          {draftError ? <p className="mt-3 text-xs font-medium text-red-600">{draftError}</p> : null}
          <button type="button" onClick={add} disabled={remainingOwnership <= 0} className="mt-5 inline-flex h-11 items-center gap-2 rounded-xl bg-[#003478] px-4 text-xs font-semibold text-white transition hover:bg-[#002b63] disabled:cursor-not-allowed disabled:bg-slate-300"><Plus className="h-4 w-4" /> {remainingOwnership <= 0 ? "100% allocated" : "Add shareholder"}</button>
        </section>

        <section>
          <SubsectionHeading title={`Saved shareholders (${form.shareholders.length})`} description={shareholderStructureSaved ? "The structure is saved. Open each available shareholder application below." : "Bring the direct ownership total to exactly 100%, then save the structure to unlock shareholder applications and the interconnection graph."} />
          <div className="space-y-3">
            {form.shareholders.length ? form.shareholders.map((shareholder, index) => {
              ...
              const typeLabel = shareholder.type === "corporate" ? "Corporate entity" : shareholder.type === "trust" ? "Trust" : "Individual";
              return (
                <div key={shareholder.id} className={`rounded-2xl border bg-white p-4 transition ${shareholderStructureSaved && required && !complete ? "border-amber-200" : "border-slate-200"}`}>
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
                    <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-slate-100 text-slate-500">{shareholder.type === "corporate" ? <Building2 className="h-4 w-4" /> : shareholder.type === "trust" ? <ShieldCheck className="h-4 w-4" /> : <UserRound className="h-4 w-4" />}</span>
                  ...
                  {shareholderStructureSaved && required && !complete ? <div className="mt-3 flex items-start gap-2 rounded-xl bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-800"><GitBranch className="mt-0.5 h-3.5 w-3.5 shrink-0" />This {typeLabel.toLowerCase()} owns at least 25%. Complete its profile and every required ownership layer.</div> : null}
                </div>
              );
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
          <SubsectionHeading title="Save the direct shareholder structure" description="The shareholder count and ownership total are calculated automatically. At exactly 100%, save the structure to unlock the graph and shareholder applications." />
          <div className="grid gap-5 sm:grid-cols-2">
          ...
          <div className={`mt-5 flex flex-col gap-4 rounded-2xl border p-4 sm:flex-row sm:items-center sm:justify-between ${form.company.shareholdersConfirmed ? "border-emerald-200 bg-emerald-50/70" : "border-slate-200 bg-slate-50"}`}>
            <div className="flex items-start gap-3">{form.company.shareholdersConfirmed ? <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-emerald-700" /> : <GitBranch className="mt-0.5 h-5 w-5 shrink-0 text-[#003478]" />}<div><p className="text-sm font-semibold text-slate-950">{form.company.shareholdersConfirmed ? "Direct structure saved" : "Ready to save?"}</p><p className="mt-1 text-xs leading-5 text-slate-600">{form.company.shareholdersConfirmed ? "The total is 100%. The overall graph is shown below and shareholder application actions are now available above." : "Complete each shareholder record and bring direct ownership to exactly 100%."}</p></div></div>
        ...
        {shareholderStructureSaved ? <OwnershipHierarchyGraph
