...
export function CompanyDirectorsStep({
  ...
  const updateDraft = <K extends keyof CompanyDirector>(
    key: K,
    value: CompanyDirector[K],
  ) => {
    setDraft((current) => ({ ...current, [key]: value }));
    setDraftErrors((current) => ({ ...current, [key]: undefined }));
  };
  const add = () => {
    const nextErrors = getDirectorDraftErrors(draft);
...
export function CompanyShareholdersStep({
  ...
  const editingShareholder = form.shareholders.find(
    (shareholder) => shareholder.id === editingId,
  );
  const allocatedExcludingDraft =
    shareholderPercentageTotal - (Number(editingShareholder?.percentage) || 0);
  const remainingOwnership = Math.max(0, 100 - allocatedExcludingDraft);
  const remainingOwnershipLabel = remainingOwnership
    .toFixed(2)
    .replace(/\.00$/, "");

  const updateDraft = (patch: Partial<CompanyShareholder>) => {
  ...
  const beginEdit = (shareholder: CompanyShareholder) => {
    setDraft({ ...shareholder });
    setDraftErrors({});
    setEditingId(shareholder.id);
  };

  const add = () => {
    const nextErrors = getShareholderDraftErrors(draft, remainingOwnership);
    setDraftErrors(nextErrors);
    if (Object.keys(nextErrors).length) return;
    if (editingId) {
      updateShareholder(draft);
      updateCompany("shareholdersConfirmed", false);
    } else {
  ...
  return (
    <div className="animate-[fadeUp_.35s_ease-out]">
      ...
      <div className="space-y-8">
        ...
        <section className="rounded-2xl border border-slate-200 bg-slate-50/55 p-5 sm:p-6">
          <SubsectionHeading
            title={editingId ? "Edit shareholder" : "Add a shareholder"}
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {draft.type === "corporate" ? (
              <Field
                ...
                htmlFor="shareholderCompanyType"
            ...
            >
              <input
                ...
                disabled={!editingId && remainingOwnership <= 0}
                    ...
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            if (editingId === shareholder.id) resetDraft();
