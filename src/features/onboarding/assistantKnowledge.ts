...
export function getStepGuidance(
  ...
  if (stepId === "directors")
    return "Add every director or partner with their full name, valid email and phone. The required director count is calculated automatically after records are saved. Then select one saved director as the default communication recipient. Invitations are queued and sent after submission.";
  if (stepId === "shareholders")
    return "The Shareholders section is shown only for a Private company. Add each individual, corporate entity or trust shareholder with its ownership percentage, full name, valid email and phone. Direct ownership cannot exceed 100% and must total exactly 100%, then use Save all shareholders. Saving unlocks the overall interconnection graph and each shareholder’s Fill application action. Individual applications include Personal Information, Identity, Upload Proof, E-Signature and Review. Corporate and Trust applications include their matching profile, business, related-party, ownership, document, signature and Review sections. Each recursively disclosed ownership layer must also total exactly 100% before its graph and application actions appear. Cash Accounts and Link External Account stay in the main application. A corporate entity or trust holding 25% or more must complete its application and disclose ownership recursively until an individual ultimate beneficial owner is identified.";
  if (stepId === "trustees")
...
export function getMissingItems(
  ...
  if (stepId === "directors") {
    if (!form.directors.length)
      missing.push("At least one saved director or partner");
  ...
  if (stepId === "shareholders") {
    if (!form.shareholders.length)
      missing.push("At least one saved shareholder");
    if (!hasCompletePercentageLayer(form.shareholders)) {
      missing.push(
        `Direct ownership currently totals ${percentageTotal(form.shareholders).toFixed(2).replace(/\.00$/, "")}% and must equal exactly 100%`,
      );
    }
    form.shareholders.forEach((shareholder) => {
      if (!isCompleteShareholder(shareholder)) {
        missing.push(
          `Complete the shareholder record for ${shareholder.name || "an unnamed shareholder"}`,
        );
      }
      if (
        ...
        missing.push(
          `Complete the required type-specific application and 100% ownership layers for ${shareholder.name || "a 25%-or-more entity shareholder"}`,
        );
      }
    });
    if (
