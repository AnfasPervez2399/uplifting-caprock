...
const WHY_GUIDANCE: Record<StepId, string> = {
  application:
    "The legal owner determines the people, regulatory checks and evidence required for the application.",
  personal:
    "Personal information supports identity, age, eligibility and customer due-diligence checks for the main applicant.",
  entity:
    "Registered company details allow the entity to be checked against the appropriate Australian or foreign company records.",
  trust:
    "Trust profile details identify the legal arrangement and connect it with its trustee and establishment records.",
  business:
    "Registration, address, tax, activity and funding information supports regulatory due diligence and risk assessment.",
  directors:
    "Directors, partners and controlling persons must be identified, and one person must receive company communications.",
  shareholders:
    "Shareholder records establish ownership and help identify the people or entities that control or benefit from the company.",
  trustees:
    "Trustees administer or control the trust, so their identity, contact details and corporate structure must be recorded.",
  beneficiaries:
    "Beneficiary records support verification of the people or entities holding a beneficial interest in the trust.",
  identity:
    "The selfie is compared with identity evidence to help confirm that the applicant is the person applying.",
  ...
  documents:
    "Supporting evidence allows Caprock to independently verify identity, address, registration, ownership or trust status.",
  signature:
    "The signatory record identifies the authorised person who will receive and validate the electronic signature request.",
  review:
    "Review and declarations provide a final, auditable confirmation that the application is accurate before submission.",
};
...
export function getStepGuidance(
  ...
  if (stepId === "entity") {
    if (applicationType === "australian-company")
      return "Enter the ASIC-registered company name, registration number, state or territory, company type, incorporation date, investment currency and expected investment range. The logo and website are optional, but a supplied website must be valid.";
    if (applicationType === "asic-non-australian-company")
      return "Enter the ASIC-registered company name, ARBN, investment currency and expected investment range. The logo and website are optional, but a supplied website must be valid.";
    ...
    if (isTrust) {
      const licence =
        applicationType === "regulated-trust"
          ? "No AFSL field is requested for this trust subtype in this flow."
          : "An Australian Financial Services Licence number is also required for this trust subtype.";
  ...
  if (stepId === "directors")
    return "Enter the declared number of directors or partners, then save exactly that many records. Each record needs full name, valid email and phone. Select one saved director as the default communication recipient. Invitations are queued and sent after submission.";
  if (stepId === "shareholders")
    return "Enter the declared shareholder count and save exactly that many records. Each shareholder needs individual or corporate type, ownership percentage, full name, valid email and phone; corporate shareholders also need a company structure. The supplied audit does not require percentages to total exactly 100%.";
  if (stepId === "trustees")
    return "Enter the trustee count and save exactly that many complete trustees. Each needs type, name, valid email and phone. A corporate trustee also needs its company structure and at least one complete director. Select one saved trustee as the default communication recipient.";
  if (stepId === "beneficiaries")
    return "Enter the beneficiary count and save exactly that many complete beneficiaries. Each needs type, name, valid email and phone. A corporate beneficiary also needs its company structure and at least one complete director.";
  if (stepId === "identity")
  ...
  if (stepId === "bank")
    return "Save at least one external account with bank name, SWIFT/BIC, bank address, account number or IBAN, account currency and a recent bank statement or official bank letter. Australian BSB is optional and applies to Australian accounts.";
  if (stepId === "cash")
    return "Select at least one currency. One cash account is created for each selected currency, and duplicate currencies are not allowed.";
  if (stepId === "documents") {
    ...
    const documentNames = requiredEntityDocuments
  ...
  if (stepId === "signature")
    return "Enter the authorised signatory’s full legal name, valid email, phone and date of birth. The signatory must be at least 18. The signature request is sent after the application passes initial review.";
  if (stepId === "review")
    return "Check every summary and open uploaded evidence where needed. Before secure submission, every section must be complete and both declarations—accuracy and consent—must be selected. After submission, the application remains on Review with an Under review status.";
...
export function getMissingItems(
  ...
  if (stepId === "personal") {
    ...
    if (!form.personal.dateOfBirth) missing.push("Date of birth");
      ...
      missing.push("Date of birth must show the applicant is at least 18");
    ...
    if (!form.personal.expectedInvestment)
      missing.push("Expected investment amount");
    if (isJoint) {
      if (!form.jointApplicants.length)
        missing.push("At least one joint applicant");
      form.jointApplicants.forEach((applicant, index) => {
        if (!isCompleteJointPersonal(applicant, sharedAddress))
          missing.push(`Complete joint applicant ${index + 1}`);
      });
    }
    return missing;
  }
  ...
  if (stepId === "entity") {
    ...
    if (form.company.website.trim() && !isValidWebsiteUrl(form.company.website))
      missing.push("Valid company website, or leave it blank");
    ...
    if (type === "australian-company") {
      ...
      if (!form.company.arbn.trim())
        missing.push("Australian Registered Body Number (ARBN)");
    } else if (
  ...
  if (stepId === "business" && isSoleTrader) {
    ...
    if (form.business.assessmentNature === "foreign") {
      ...
      if (
        ...
        missing.push("Description of other business activity");
      if (!form.business.sourceOfFunds.trim())
        missing.push("Source and origin of funds");
      if (!form.business.intendedTransactions.trim())
        missing.push("Intended transaction behaviour");
      if (!form.business.beneficialOwnership.trim())
        missing.push("Beneficial ownership of funds");
      if (form.business.foreignBusinessCountry === "United States") {
        ...
        if (
          ...
          missing.push("Social Security Number");
        if (!form.business.usTaxResident)
          missing.push("U.S. tax-residency status");
        if (
          form.business.usTaxResident === "yes" &&
          !form.business.taxIdentificationNumber.trim()
        )
          missing.push("U.S. tax identification number");
      }
    }
    return missing;
  }
  ...
  if (stepId === "business" && isCompany) {
    ...
    if (type === "australian-company") {
      if (!form.company.registrationType)
        missing.push("ASIC registration type");
      if (!form.company.acn.trim())
        missing.push("Australian Company Number (ACN)");
      if (!form.company.amlActivity.trim())
        missing.push("Prosecution and inquiry disclosure, or “None”");
    } else {
      if (!form.company.companyType) missing.push("Company type");
      if (!form.company.incorporationDate)
        missing.push("Date of incorporation");
      ...
      if (!form.company.principalAddress.trim())
        missing.push("Principal business address");
  ...
  if (stepId === "business" && isTrust) {
    ...
    if (!form.trust.expectedInvestment)
      missing.push("Expected investment amount");
    if (!form.trust.establishedCountry)
      missing.push("Trust establishment country");
    ...
    if (type !== "regulated-trust" && !form.trust.afsLicenseNumber.trim())
      missing.push("Australian Financial Services Licence number");
    return missing;
  }

  if (stepId === "directors") {
    if (
      !form.company.directorCount ||
      Number(form.company.directorCount) !== form.directors.length
    )
      missing.push(
        `Director count matching ${form.directors.length} saved record${form.directors.length === 1 ? "" : "s"}`,
      );
    if (!form.company.defaultRecipientId)
      missing.push("Default communication recipient");
    return missing;
  }
  if (stepId === "shareholders") {
    if (
      !form.company.shareholderCount ||
      Number(form.company.shareholderCount) !== form.shareholders.length
    )
      missing.push(
        `Shareholder count matching ${form.shareholders.length} saved record${form.shareholders.length === 1 ? "" : "s"}`,
      );
    return missing;
  }
  if (stepId === "trustees") {
    if (
      !form.trust.trusteeCount ||
      Number(form.trust.trusteeCount) !== form.trustees.length
    )
      missing.push(
        `Trustee count matching ${form.trustees.length} saved record${form.trustees.length === 1 ? "" : "s"}`,
      );
  ...
  if (stepId === "beneficiaries") {
    if (
      !form.trust.beneficiaryCount ||
      Number(form.trust.beneficiaryCount) !== form.beneficiaries.length
    )
      missing.push(
        `Beneficiary count matching ${form.beneficiaries.length} saved record${form.beneficiaries.length === 1 ? "" : "s"}`,
      );
    return missing;
  }
  if (stepId === "identity") {
    if (!form.identity.selfie)
      missing.push("Clear selfie image, 10 MB or smaller");
    return missing;
  }
  if (stepId === "bank") {
    if (!form.bankAccounts.length)
      missing.push(
        "At least one saved external bank account with verification evidence",
      );
    form.bankAccounts.forEach((account, index) => {
      if (
        ...
        missing.push(`Complete saved bank account ${index + 1}`);
    });
    return missing;
  }
  if (stepId === "cash") {
    if (!form.cashAccounts.length)
      missing.push("At least one cash-account currency");
    return missing;
  }
  if (stepId === "documents") {
    if (!isIndividual) {
    ...
    proofApplicantProfiles.forEach((applicant) => {
      ...
      if (!hasPhotoIdentity(documents, applicant.country))
        missing.push(`${applicant.label}: required photo identification`);
      if (!hasAddressEvidence(documents))
        missing.push(`${applicant.label}: residential address evidence`);
      if (!hasPersonalDetailsEvidence(documents))
        missing.push(`${applicant.label}: current CV or valid website URL`);
    });
    return missing;
  }
  if (stepId === "signature") {
    if (!form.signature.name.trim())
      missing.push("Authorised signatory’s full name");
    if (!isValidEmail(form.signature.email))
      missing.push("Valid signatory email address");
    ...
    if (!form.signature.dateOfBirth) missing.push("Signatory date of birth");
      ...
      missing.push("Signatory must be at least 18");
    return missing;
  }
  if (stepId === "review") {
    visibleSteps
      .filter((step) => step.id !== "review" && !completion[step.id])
      .forEach((step) =>
        missing.push(`${ASSISTANT_STEP_NAMES[step.id]} section`),
      );
